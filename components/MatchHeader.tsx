...
const MatchHeader: FC<MatchHeaderInterface> = ({ match }) => (
  <div className='h-max inline-flex w-full justify-between px-24 items-center bg-[#013a54] text-white py-1.5'>
    <span className='capitalize font-bold'>{match.sede} - {new Date(match.date).toLocaleString(['es-ES'], { weekday: 'short', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
