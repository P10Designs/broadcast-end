...
import { Clock, Period, MainModal, Penalties } from 'components/Controller/Modals'
  ...
  <div className='w-full h-full absolute z-10 bg-black/60 flex items-center justify-center'>
    <div className={'flex flex-col items-center justify-start bg-white rounded ' + (modalType !== 'goal' && modalType !== 'fault' && modalType !== 'penalties' ? 'w-1/3' : 'w-max')}>
      ...
      <div>
        ...
        {modalType !== 'penalties' &&
          <button onClick={() => { handleSave() }} className='mx-1.5 bg-blue-500 px-2 py-1 rounded-md inline-flex uppercase text-xs font-bold items-center justify-center hover:bg-blue-700 transition-all'>
            <EditIcon />
