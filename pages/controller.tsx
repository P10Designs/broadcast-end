...
const Controller: NextPage<ControllerProps> = ({ match, players }) => {
  ...
  const handleSave = () => {
    if (modalType === 'clock') {
      ...
      if (team === 'local') {
        ...
        let assist = ''
        if (changed.first !== '') {
          const split = getObjects(localPlayers, 'dorsal', changed.first)[0].name.split(',')
          goal = `${split[1]} ${split[0].split(' ')[0]}`
          for (let i = 0; i < localPlayers.length; i++) {
            if (localPlayers[i].dorsal === changed.first) {
              localPlayers[i].matchStats.g += 1
            }
          }
        }
        if (changed.second !== '') {
          const split = getObjects(localPlayers, 'dorsal', changed.second)[0].name.split(',')
          assist = `${split[1]} ${split[0].split(' ')[0]}`
          for (let i = 0; i < localPlayers.length; i++) {
            if (localPlayers[i].dorsal === changed.second) {
              localPlayers[i].matchStats.a += 1
            }
          }
        }
        if (data.local.fault1 !== '') data.local.fault1 = '00:00'
        data.local.string = 'G: ' + goal + (changed.second !== '' ? `, A: ${assist}` : '')
      } else {
        if (data.period.big === 'PERIODO 1') {
        ...
        let assist = ''
        if (changed.first !== '') {
          const split = getObjects(visitorPlayers, 'dorsal', changed.first)[0].name.split(',')
          goal = `${split[1]} ${split[0].split(' ')[0]}`
          for (let i = 0; i < visitorPlayers.length; i++) {
            if (visitorPlayers[i].dorsal === changed.first) {
              visitorPlayers[i].matchStats.g += 1
            }
          }
        }
        if (changed.second !== '') {
          const split = getObjects(visitorPlayers, 'dorsal', changed.second)[0].name.split(',')
          assist = `${split[1]} ${split[0].split(' ')[0]}`
          for (let i = 0; i < visitorPlayers.length; i++) {
            if (visitorPlayers[i].dorsal === changed.second) {
              visitorPlayers[i].matchStats.a += 1
            }
          }
        }
        if (data.visitor.fault1 !== '') data.visitor.fault1 = '00:00'
        data.visitor.string = 'G: ' + goal + (changed.second !== '' ? `, A: ${assist}` : '')
      }
      ...
      if (team === 'local') {
        ...
        data.local.string = `${fault}, F: ${changed.second}`
      } else if (team === 'visitor') {
        if (data.period.big === 'PERIODO 1') {
        ...
        const split = getObjects(visitorPlayers, 'dorsal', changed.first)[0].name.split(',')
        const fault = `${split[1]} ${split[0].split(' ')[0]}`
        data.visitor.string = `${fault}, F: ${changed.second}`
      }
    }
  }
  ...
  return (
    <div className='min-h-screen h-full bg-gray-300'>
      ...
      {view &&
        <Menu setView={setView} handleSave={handleSave} modalType={modalType} changed={changed} players={team === 'local' ? localPlayers : visitorPlayers} />
      }
      <div className='inline-flex flex-wrap w-full h-special-1 items-start justify-start'>
        <TeamComponent selected={selected} team='local' match={match.local} players={localPlayers} playerSelected={playerSelected.local} setPlayerSelected={playerHandler}/>
        <div className='w-1/5 h-full flex flex-col items-start justify-start'>
          ...
          <Data name='SAQUES' local={data.local.saquesTotal} visitor={data.visitor.saquesTotal} />
          <Data name='POSESION' local={data.local.posTotal + '%'} visitor={data.visitor.posTotal + '%'} />
          <Fault local={data.local.fault} visitor={data.visitor.fault} />
          <Selector selected={selected} setSelected={setSelected} />
        </div>
        <TeamComponent selected={selected} team='visitor' match={match.visitor} players={visitorPlayers} playerSelected={playerSelected.visitor} setPlayerSelected={playerHandler}/>
      </div>
